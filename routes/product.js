...
product_router.get("/category/:cate_id", (req, res) => {
  mysqlConnection.query(
    "SELECT products.product_id , product_name, product_detail, product_unit, product_price, product_inventory, product_image FROM products join product_thumbnail on products.product_id = product_thumbnail.product_id WHERE category_id = ?", // missing product_thumbnail
    [req.params.cate_id],
    (err, rows, fields) => {
      if (err) {
        console.log("fail to query: " + err);
